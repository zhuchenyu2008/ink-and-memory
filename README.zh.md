...
## 适用人群
- 在校大学生、初入职场的年轻人，压力较大，需要低门槛的情绪疏导。
- 喜欢写日记/手帐，但觉得传统日记时间和心理成本太高。
- 想记录生活，但希望更轻量。
- 文学爱好者、创业者，需要灵感火花。
...
## 路线图（摘要）
- 按用户的时间线调度与好友时区提示。
- 访客“影子账号”模式（匿名 JWT，同一后端路径、配额限制）。
- 开源完善：控制面接口文档、lint、种子数据。
- 用户引导与社交设计升级。
- 设计主题收敛，内置提示词迭代；模型选型/切换/回退预案。
- 时间线 UI 重构，反思可视化更轻盈。
- 后端延迟优化：监控瓶颈、缓存/批处理。
- 更细致的图片与资产：艺术疗愈思路、自动 Agent 贴纸、问候 Banner（Gemini 3）。
- 内容与评测：名人经历搜索用于 Agent，卡组自动评测，Agent 长记忆。
- 声音层：补语音反馈与 BGM。
- 工作流与测试覆盖完善。
